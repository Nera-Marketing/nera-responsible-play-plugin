...
#
# Usage:
#   ./release.sh          # reads version from nera-responsible-play-plugin.php (Version header)
...
# Pushes to `origin` (configure it once: git remote add origin git@github.com:Nera-Marketing/nera-responsible-play-plugin.git).
#
...
#   3. Copies plugin to a clean temp dir; sets * Version + NERA_RP_VERSION; syncs readme Stable tag + plugin.json
...
PLUGIN_SLUG="nera-responsible-play-plugin"
GITHUB_REPO="Nera-Marketing/nera-responsible-play-plugin"
...
# (Self-skips when no logo.png is present.)
...
echo "▶ Setting * Version + NERA_RP_VERSION to ${VERSION} in release tree (for PUC)..."
...
if sed --version >/dev/null 2>&1; then
  ...
  sed -i "s/define( 'NERA_RP_VERSION', '[^']*' );/define( 'NERA_RP_VERSION', '${VERSION}' );/" "$PHP_MAIN"
else
  sed -i '' "s/^ \\* Version: .*/ * Version: ${VERSION}/" "$PHP_MAIN"
  sed -i '' "s/define( 'NERA_RP_VERSION', '[^']*' );/define( 'NERA_RP_VERSION', '${VERSION}' );/" "$PHP_MAIN"
fi
